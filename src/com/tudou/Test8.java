...
/*模拟双色球：业务分析、用户投注一组号码
* 介绍业务：投注号码由6个红色求号码和个蓝色求号码组成。红色球号码从1-33中选择；蓝色号码从1-16选择
* 奖等    红球   蓝球    奖金
* 一      6      1     最高100万
* 二      6            最高500万
* 三      5      1     3000元
...
public class Test8 {
    ...
    /**1、设计一个方法，用于让用户投注一组号码并返回（前6个是红色号码，最后1个是蓝色号码）*/
    public static int[] userSelectNumbers(){
        // 2、创建一个整型数组，用于储存用户投注的7个号码（前6个是红球号码，最后1个是蓝球号码）
        ...
        while (true){
            ...
            if (number < 1 || number > 16){
                System.out.println("对不去，您输入的蓝球号码范围不对！");
            }else {
                numbers[6] = number;
                break;
            }
    ...
    public static int[] createLuckNumbers(){
        ...
        for (int i = 0;i<numbers.length -1 ;i++){
            ...
            while (true){
                //3、为当前这个位置随机一个红色号码出来存入。1-33 ==>-1===>(1,32)+1
                int number = r.nextInt(33)+1;

                //4、判断这个号码是否之前出现过（红色号码不能重复）。
                if (!exist(numbers,number)){
                    numbers[i] = number;
                    break;
                }
    ...
    public static void judge(int[] userNumbers , int[] luckNumbers){
        ...
        if (redCount == 6 && blueCount == 1){
            ...
            System.out.println("恭喜您，中奖3000元，可以出去吃一顿小龙虾了~");
